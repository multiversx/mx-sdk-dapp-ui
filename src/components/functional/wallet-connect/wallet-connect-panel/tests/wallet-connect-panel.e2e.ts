...
import { WalletConnectFlow } from '../../wallet-connect-flow/wallet-connect-flow';
import { WalletConnectPanel } from '../../wallet-connect-panel/wallet-connect-panel';
...
describe('wallet-connect-panel', () => {
  const createPage = async (props: { data?: any; isOpen?: boolean } = {}) => {
    const page = await newSpecPage({
      components: [WalletConnectPanel, WalletConnectFlow],
      html: `<${tag}></${tag}>`,
    });
